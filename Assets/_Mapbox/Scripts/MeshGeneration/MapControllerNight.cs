...
{
	public class MapControllerNight : MonoBehaviour
	{
		...
		public void Execute(double lat, double lng, int zoom, Vector4 frame)
		{
			...
			_root = new GameObject("worldRootNight");
			...
			for (int i = (int)(tms.x - frame.x); i <= (tms.x + frame.z); i++)
			{
				...
				{
					var tile = new GameObject("TileNight - " + i + " | " + j).AddComponent<UnityTile>();
		...
		public void Request(Vector2 pos, int zoom)
		{
			...
			{
				var tile = new GameObject("TileNight - " + pos.x + " | " + pos.y).AddComponent<UnityTile>();
				_tiles.Add(pos, tile);
				tile.transform.SetParent(_root.transform, false);
				tile.Zoom = zoom;
